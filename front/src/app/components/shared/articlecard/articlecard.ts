...
export class ArticleCardComponent {
  ...
  sendInteracionClick(): void {
    this.interactionService
      .postInteraction(this.article.category, 'CLICK', this.article.id)
  ...
  cancelFavoriteM(): void {
    ...
    this.feedService.cancelFavorite(cancelPayload).subscribe({
      next: () => {
        ...
        this.feedService.favoriteRemoved$.next(externalId);
      },
      error: (err) => {
  ...
  private updateSessionCache(): void {
    ...
    const updated = articles.map((a: any) => (a.id === this.article.id ? this.article : a));
